...
public sealed class UserService : IUserService
{
    ...
    public UserService(IAuthService authService, IUnitOfWork unitOfWork, IUserFactory userFactory, IUserRepository userRepository)
    {
    ...
    {
        IResult validation = new AddUserModelValidator().Validation(model);
        ...
        IResult<Auth> auth = await _authService.AddAsync(model.Auth);
        ...
        User user = _userFactory.Create(model, auth.Data);
    ...
    {
        long authId = await _userRepository.GetAuthIdByUserIdAsync(id);
    ...
    {
        IResult validation = new UpdateUserModelValidator().Validation(model);
        ...
        User user = await _userRepository.GetAsync(model.Id);
